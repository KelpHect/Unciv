    ...
    pub async fn reconcile_authoritative_state(&self) -> Result<ReconciliationReport, CommitError> {
        ...
        let structural_sql = r#"
            ...
            WHERE (r.revision=0 AND (r.revision_kind<>'genesis' OR r.parent_revision IS NOT NULL OR r.command_id IS NOT NULL))
               ...
               OR (r.revision>0 AND r.revision_kind IN ('recovery','rewind','lobby_reconfiguration') AND r.command_id IS NOT NULL)
               OR r.revision_kind NOT IN ('genesis','command','recovery','rewind','lobby_reconfiguration')
            ...
            WHERE r.revision>0 AND (
                ...
                OR
                (r.revision_kind='lobby_reconfiguration' AND (
                  (SELECT count(*) FROM game_outbox o
                   WHERE o.game_id=r.game_id AND o.revision=r.revision AND o.topic='game.lobby.reconfigured')
                  +(SELECT count(*) FROM game_outbox_receipts receipt
                    WHERE receipt.game_id=r.game_id AND receipt.revision=r.revision AND receipt.topic='game.lobby.reconfigured')
                )<>1)
            )
            UNION ALL
            ...
            WHERE o.topic IN ('game.revision.committed','game.revision.recovered','game.revision.rewound','game.lobby.reconfigured') AND r.game_id IS NULL
            UNION ALL
            ...
            WHERE receipt.topic IN ('game.revision.committed','game.revision.recovered','game.revision.rewound','game.lobby.reconfigured') AND r.game_id IS NULL
            UNION ALL
