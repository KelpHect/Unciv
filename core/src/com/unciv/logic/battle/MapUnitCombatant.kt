...
class MapUnitCombatant(val unit: MapUnit) : ICombatant {
    ...
    override fun getAttackingStrength(defender: ICombatant?): Int {
        ...
        val extraStrength = unit.matchingUniquesSequence(UniqueType.StrengthAmount, state).sumOf { it.params[0].toInt() }
    ...
    fun getMatchingUniques(uniqueType: UniqueType, gameContext: GameContext, checkCivUniques: Boolean): Sequence<Unique> =
        unit.matchingUniquesSequence(uniqueType, gameContext, checkCivUniques)

    @Readonly
    fun matchingUniquesSequence(uniqueType: UniqueType, gameContext: GameContext, checkCivUniques: Boolean): Sequence<Unique> =
        unit.matchingUniquesSequence(uniqueType, gameContext, checkCivUniques)
