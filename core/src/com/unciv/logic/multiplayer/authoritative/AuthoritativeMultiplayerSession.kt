...
class AuthoritativeMultiplayerSession(
    ...
    suspend fun moveUnitTowardIfOpen(
        ...
        return when (val current = bus.state) {
            ...
            is AuthoritativeSyncState.Synchronized ->
                bus.moveUnitToward(unitId, destinationX, destinationY, escortUnitId)
            else -> {
                bus.refresh()
                bus.moveUnitToward(unitId, destinationX, destinationY, escortUnitId)
            }
    ...
    suspend fun manageConstructionQueuesIfOpen(
        gameId: String,
        cityId: String,
        constructionName: String,
        queueIndex: Int?,
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.ManageConstructionQueues &&
                    current.pending.cityId == cityId &&
                    current.pending.constructionName == constructionName &&
                    current.pending.queueIndex == queueIndex &&
                    current.pending.action == action) {
            ...
            is AuthoritativeSyncState.Synchronized ->
                bus.manageConstructionQueues(cityId, constructionName, queueIndex, action)
            else -> {
                bus.refresh()
                bus.manageConstructionQueues(cityId, constructionName, queueIndex, action)
            }
    ...
    suspend fun buyCityTileBatchIfOpen(
        gameId: String,
        cityId: String,
        ring: Int,
    ): AuthoritativeCommandOutcome? {
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.BuyCityTileBatch &&
                    current.pending.cityId == cityId && current.pending.ring == ring) {
                    "Resolve the pending authoritative command before buying another tile batch"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized -> bus.buyCityTileBatch(cityId, ring)
            else -> {
                bus.refresh()
                bus.buyCityTileBatch(cityId, ring)
            }
        }
    }
    ...
    suspend fun resolveCityDispositionIfOpen(
        gameId: String,
        cityId: String,
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.ResolveCityDisposition &&
                    current.pending.cityId == cityId && current.pending.action == action) {
                    "Resolve the pending authoritative command before choosing another disposition"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized -> bus.resolveCityDisposition(cityId, action)
            else -> {
                bus.refresh()
                bus.resolveCityDisposition(cityId, action)
            }
        }
    }
    ...
    suspend fun giftUnitIfOpen(gameId: String, unitId: Int): AuthoritativeCommandOutcome? {
        val bus = mutex.withLock { games[gameId] } ?: return null
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.GiftUnit && current.pending.unitId == unitId) {
                    "Resolve the pending authoritative command before gifting another unit"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized -> bus.giftUnit(unitId)
            else -> { bus.refresh(); bus.giftUnit(unitId) }
        }
    }
    ...
    suspend fun transformUnitIfOpen(
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.TransformUnit &&
                    current.pending.unitId == unitId &&
                    current.pending.actionId == actionId) {
                    "Resolve the pending authoritative command before transforming another unit"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized -> bus.transformUnit(unitId, actionId)
            else -> { bus.refresh(); bus.transformUnit(unitId, actionId) }
        }
    }

    suspend fun triggerUnitUniqueIfOpen(
        gameId: String,
        unitId: Int,
        actionId: String,
    ): AuthoritativeCommandOutcome? {
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.TriggerUnitUnique &&
                    current.pending.unitId == unitId && current.pending.actionId == actionId) {
                    "Resolve the pending authoritative command before triggering another unit action"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized -> bus.triggerUnitUnique(unitId, actionId)
            else -> { bus.refresh(); bus.triggerUnitUnique(unitId, actionId) }
        }
    }

    suspend fun createInstantImprovementIfOpen(
        gameId: String,
        unitId: Int,
        actionId: String,
    ): AuthoritativeCommandOutcome? {
        ...
        return when (val current = bus.state) {
            is AuthoritativeSyncState.Retryable -> {
                check(current.pending is PendingAuthoritativeCommand.CreateInstantImprovement &&
                    current.pending.unitId == unitId && current.pending.actionId == actionId) {
                    "Resolve the pending authoritative command before creating another improvement"
                }
                bus.retryPending()
            }
            is AuthoritativeSyncState.Synchronized ->
                bus.createInstantImprovement(unitId, actionId)
            else -> error("Authoritative game is not synchronized")
        }
    }
