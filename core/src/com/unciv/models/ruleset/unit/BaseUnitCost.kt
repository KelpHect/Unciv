...
class BaseUnitCost(val baseUnit: BaseUnit) {
    ...
    fun getProductionCost(civInfo: Civilization, city: City?): Int {
        ...
        for (unique in baseUnit.matchingUniquesSequence(UniqueType.CostIncreasesPerCity, stateForConditionals))
            productionCost += civInfo.cities.size * unique.params[0].toInt()

        for (unique in baseUnit.matchingUniquesSequence(UniqueType.CostIncreasesWhenBuilt, stateForConditionals))
            productionCost += civInfo.civConstructions.builtItemsWithIncreasingCost[baseUnit.name] * unique.params[0].toInt()

        for (unique in baseUnit.matchingUniquesSequence(UniqueType.CostPercentageChange, stateForConditionals))
    ...
    fun canBePurchasedWithStat(city: City, stat: Stat): Boolean {
        ...
        if (city.matchingUniquesSequence(UniqueType.BuyUnitsIncreasingCost, conditionalState)
                    .any {
        ...
        if (city.matchingUniquesSequence(UniqueType.BuyUnitsByProductionCost, conditionalState)
                    .any { it.params[1] == stat.name && baseUnit.matchesFilter(it.params[0], conditionalState) }
        )
            return true

        if (city.matchingUniquesSequence(UniqueType.BuyUnitsWithStat, conditionalState)
                    .any {
        ...
        if (city.matchingUniquesSequence(UniqueType.BuyUnitsForAmountStat, conditionalState)
                    .any {
                        it.params[2] == stat.name
                                && baseUnit.matchesFilter(it.params[0], conditionalState)
                                && city.matchesFilter(it.params[3])
    ...
    fun getStatBuyCost(city: City, stat: Stat): Int? {
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.BuyUnitsDiscount)) {
    ...
    fun getBaseBuyCosts(city: City, stat: Stat): Sequence<Float> {
        val conditionalState = city.state
        return sequence {
            yieldAll(city.matchingUniquesSequence(UniqueType.BuyUnitsIncreasingCost, conditionalState)
                .filter {
                    it.params[2] == stat.name
                            && baseUnit.matchesFilter(it.params[0], conditionalState)
                            && city.matchesFilter(it.params[3])
            ...
            yieldAll(city.matchingUniquesSequence(UniqueType.BuyUnitsByProductionCost, conditionalState)
            ...
            yieldAll(city.matchingUniquesSequence(UniqueType.BuyUnitsForAmountStat, conditionalState)
                .filter {
                    it.params[2] == stat.name
                            && baseUnit.matchesFilter(it.params[0], conditionalState)
                            && city.matchesFilter(it.params[3])
