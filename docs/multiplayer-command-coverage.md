...
| City tile acquisition and tile-specific construction | `BuyCityTile(cityId, x, y)` and `QueueConstructionAtTile(cityId, constructionName, x, y)` implemented | Shared `CityExpansionManager.buyTile` and `CityConstructions.addToQueue(..., tile)` through `HeadlessGameEngine`; tile-targeted purchases remain separate and fail closed | Membership-derived civilization, current turn, city ownership, canonical map coordinate, canonical placement/ownership/range rules, constructability, treasury and server-recomputed tile price; wire contracts contain no actor, price, or client legality claim | Accepted commands refresh treasury/queue and commit canonical ownership or improvement marker; projection still lacks owned/working-city tile assignments, purchase costs, and server-projected legal construction targets | `CityScreen` routes single-tile purchases and non-purchase tile-targeted construction through the v3 session without local mutation. Multi-tile buying is disabled and tile-targeted building purchases remain fail closed; local/hotseat/legacy behavior is unchanged | **Partial** |
