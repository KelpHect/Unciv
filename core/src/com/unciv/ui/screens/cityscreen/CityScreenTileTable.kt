...
class CityScreenTileTable(private val cityScreen: CityScreen) : Table() {
        ...
        if (tileView.isCityCenter()) {
            ...
            if (otherCity != null && otherCity != cityView && otherCity.isSameCivAs(cityView) && !cityScreen.isSpying)
                innerTable.add("Move to city".toTextButton().onClick { cityScreen.game.replaceCurrentScreen(
                    CityScreen(CityView(otherCity.getCity(), cityView.viewer, cityView.spectatorMode))
                ) })
        }
    ...
    private inner class TileBuyMenu(buyTileButton: TextButton) : AnimatedMenuPopup(stage, buyTileButton) {
        ...
        @Readonly private fun getRing(ring: Int) = cityView.centerTile().tilesInDistanceSequence(ring).filter { it.owningCity() == null }
