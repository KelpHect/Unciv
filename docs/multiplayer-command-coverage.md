...
`RemoveConstruction`, `MoveConstruction`, `PurchaseConstruction`,
...
| City production queue | `QueueConstruction`, `RemoveConstruction`, and `MoveConstruction` implemented; add-to-top/all-cities, `SetPerpetualConstruction`, tile-specific construction, and bounded batch variants remain | Shared `CityConstructions` operations through `HeadlessGameEngine`; unsupported variants remain hidden/fail closed in explicitly opened v3 city UI | Membership-derived civilization, current turn, city ownership, bounded indices/names, canonical constructability/capacity, and an expected queue-entry name that prevents a stale index from targeting another item | Own city projection includes the ordered queue and currently constructable building/unit names; progress, costs, tile-specific choices and public wonder events remain | `CityConstructionsTable` routes ordinary append, one-position reorder, and removal through the v3 session/command bus without local mutation. Context-menu batches, perpetual and tile-specific controls remain suppressed; local, hotseat, and legacy games retain existing behavior | **Partial** |
