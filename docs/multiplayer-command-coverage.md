...
| Citizen and specialist assignment | `SetCityTileAssignment(cityId, x, y, assignment)` implemented with closed `unworked`/`worked`/`locked` states; `SetSpecialistCount(cityId, specialistName, count)` implemented as an absolute target; `SetCityAutomation` remains | Shared city worked/locked collections and specialist allocations through focused `HeadlessGameEngine` handlers; the engine recomputes city stats after each accepted change | Membership-derived civilization, current turn, city ownership, non-puppet/non-resistance state, canonical tile legality or specialist identity/capacity, and canonical free population; client population, capacity, yields, and legality claims are absent | Projection v7 exposes each owned city's allowlisted assignable coordinates plus canonical specialist current/capacity pairs and manual-specialist state; yields and broader automation/focus flags remain | `CityScreen` worked-icon click/double-click, tile lock/unlock, and specialist `+`/`-` route opened v3 games through the session/command bus without local mutation. The auto/manual specialist toggle is suppressed for v3 until its separate automation command exists; local, hotseat, and legacy behavior is unchanged | **Partial** |
