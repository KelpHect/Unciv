...
impl PostgresGameRepository {
    pub async fn execute_create_instant_improvement(
        &self,
        ...
        let (unit_id, action_id) = match &envelope.command {
            crate::GameCommand::CreateInstantImprovement { unit_id, action_id } => {
        ...
        let proposal = worker
            .create_instant_improvement(
                ...
                &state.snapshot,
                CreateInstantImprovementIntent {
            ...
            .map_err(|error| match error {
                ...
                other => {
                    eprintln!(
                        "authoritative worker instant-improvement transport/protocol failure: {other}"
                    );
