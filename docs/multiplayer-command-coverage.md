...
| City production queue | `QueueConstruction` implemented for ordinary append; `RemoveConstruction`, `MoveConstruction`, add-to-top/all-cities, `SetPerpetualConstruction`, tile-specific construction, and bounded batch variants remain | `CityConstructions.addToQueue` through `HeadlessGameEngine.queueConstruction`; unsupported variants remain local-domain operations but are hidden/fail closed in explicitly opened v3 city UI | Queue command derives civilization, checks current turn, city ownership, name bounds, canonical constructability and capacity; remaining variants need equivalent checks | Own city projection includes queue and currently constructable building/unit names; progress, costs, tile-specific choices and public wonder events remain | `CityConstructionsTable` routes ordinary selection through the v3 session/command bus without local mutation. Queue reorder/removal/context-menu batches, purchases, and tile-specific construction controls are suppressed for opened v3 games; local, hotseat, and legacy games retain existing behavior | **Partial** |
