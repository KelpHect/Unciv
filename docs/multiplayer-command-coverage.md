...
| Citizen and specialist assignment | `SetCityTileAssignment`, absolute `SetSpecialistCount`, boolean `SetManualSpecialists`, and zero-option `ResetCitizens(cityId)` implemented; avoid growth and citizen focus remain separate commands | Shared city worked/locked collections, specialist allocations, `reassignPopulation()`, and `reassignPopulation(resetLocked = true)` through focused `HeadlessGameEngine` handlers | Membership-derived civilization, current turn, city ownership, non-puppet/non-resistance state, canonical tile legality or specialist identity/capacity, and canonical free population; client population, capacity, focus, yields, and legality claims are absent | Projection v7 exposes each owned city's allowlisted tiles plus specialist current/capacity pairs and manual-specialist state; avoid-growth and focus flags remain | Opened v3 tile, specialist, specialist-mode, and reset controls route through the session/command bus without local mutation. Reset clears canonical locks and reassigns on the worker. Avoid-growth and focus controls remain unsupported for v3; local, hotseat, and legacy behavior is unchanged | **Partial** |
