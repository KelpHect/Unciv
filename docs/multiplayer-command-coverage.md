...
API v3 currently exposes `JoinGame`, `MoveUnit`, `QueueConstruction`,
`RemoveConstruction`, `MoveConstruction`, `SetPerpetualConstruction`, `PurchaseConstruction`,
...
| City production queue | `QueueConstruction`, `RemoveConstruction`, `MoveConstruction`, and `SetPerpetualConstruction` implemented; add-to-top/all-cities, tile-specific construction, and bounded batch variants remain | Shared `CityConstructions` operations through `HeadlessGameEngine`; perpetual choices use a distinct replacement command rather than weakening ordinary append validation; unsupported variants remain hidden/fail closed in explicitly opened v3 city UI | Membership-derived civilization, current turn, city ownership, bounded indices/names, canonical constructability/capacity, buildable perpetual choice, and an expected queue-entry name that prevents a stale index from targeting another item | Own city projection includes the ordered queue and currently available ordinary/perpetual construction names; progress, costs, tile-specific choices and public wonder events remain | `CityConstructionsTable` routes ordinary append, perpetual replacement, one-position reorder, and removal through the v3 session/command bus without local mutation. Context-menu batches and tile-specific controls remain suppressed; local, hotseat, and legacy games retain existing behavior | **Partial** |
| City purchases | `PurchaseConstruction(cityId, constructionName, currencyName, queueIndex?)` implemented for non-tile-targeted constructions | Shared `CityConstructions.purchaseConstruction` through `HeadlessGameEngine.purchaseConstruction` | Membership-derived civilization, current turn, city ownership, bounded canonical names/index, supported currency, construction eligibility, purchase availability, resources and server-recomputed cost; actor and price are absent from the wire contract | Accepted command refreshes the projection after canonical treasury/faith/resource, unit/building, queue and city-stat changes; projected costs and tile choices remain | `BuyButtonFactory.kt` routes purchases for explicitly opened v3 games through the session/command bus and performs no local purchase. Tile-targeted purchases fail closed; local, hotseat and legacy games retain existing behavior | **Partial** |
...
| City tile acquisition and tile-specific construction | `BuyCityTile(cityId, x, y)` implemented; `ChooseConstructionTile` remains | Shared `CityExpansionManager.buyTile` through `HeadlessGameEngine.buyCityTile`; tile-targeted construction logic remains local-only and hidden for v3 | Membership-derived civilization, current turn, city ownership, canonical map coordinate, adjacency/range/ownership/resistance rules, treasury and server-recomputed price; the wire contains no actor or price | Accepted tile purchase refreshes projected treasury and commits canonical tile ownership, but the current projection does not yet expose owned/working-city tile assignments or purchase costs; those and construction-target choices remain gaps | `CityScreen.askToBuyTile` routes a single selected tile through the v3 session without local mutation, marks the cache stale, and closes for reconciliation. The local multi-tile context menu is disabled for explicitly opened v3 games; tile-targeted building construction/purchase remains fail closed | **Partial** |
