...
class ThreatManagerTests {
    ...
    fun `Dangerous tiles`() {
        ...
        assertEquals(null, testGame.getTile(3,0).tilesInDistanceSequence(1).firstOrNull {tile -> !dangerousTiles.contains(tile)})
        assertEquals(null, testGame.getTile(-3,0).tilesInDistanceSequence(2).firstOrNull {tile -> !dangerousTiles.contains(tile)})
    }
