...
impl PostgresGameRepository {
    pub async fn execute_set_city_governance(
        &self,
        ...
        let (city_id, action) = match &envelope.command {
            crate::GameCommand::SetCityGovernance { city_id, action } => (city_id.clone(), *action),
        ...
        let proposal = worker
            ...
            .map_err(|error| match error {
                ...
                other => {
                    eprintln!(
                        "authoritative worker SetCityGovernance transport/protocol failure: {other}"
                    );
