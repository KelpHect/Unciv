...
| Unit movement | `MoveUnit(unitId, destination)` implemented for exact current-turn movement; persistent multi-turn orders and friendly-unit swapping remain separate typed commands | `UnitMovement.moveToTile` through `HeadlessGameEngine.moveUnit` | Membership-derived current actor/turn; projected own-unit ID and explored destination preflight; canonical ownership, bounds, current-turn reachability, movement, terrain, and occupancy legality | Stable unit IDs, explored map coordinates, resulting positions/movement, and visibility changes; server-projected legal destinations remain | Opened-v3 world-map overlay clicks and single-tap movement submit the exact current-turn destination through the session/command bus and never move the local `MapUnit`; multi-unit moves serialize command-by-command. Unsupported friendly swapping fails closed. Local, hotseat, and legacy behavior is unchanged | **Partial: multi-turn orders, swap, and projected legal destinations remain** |
