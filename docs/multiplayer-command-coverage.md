...
| City production queue | `QueueConstruction`, `RemoveConstruction`, `MoveConstruction`, `SetPerpetualConstruction`, batch variants with explicit city IDs | `CityConstructions.addToQueue/removeFromQueue` | City ownership; constructability; queue index/version; wonder/resource rules; bounded batch | Own city queue/progress and public wonder events | `CityScreenConstructionMenu.kt`, `CityConstructionsTable.kt`, `CityScreen.kt` mutate queues directly | **Missing** |
...
| Research selection | `SetCurrentResearch`, `QueueTechnology`, `RemoveQueuedTechnology`, `ChooseFreeTechnology` | civilization tech manager through `TechPickerScreen.kt` | Actor/turn; prerequisites; research agreement/free-tech grants; queue validity | Own research queue/progress and legally public researched tech/events | `TechPickerScreen.kt` changes local technology choices; no v3 variant | **Missing** |
...
resources/improvements; movement/action availability; city population, yields,
queues and purchase costs; research/policies/religion; diplomacy, trades and
