...
| Citizen and specialist assignment | `SetCityTileAssignment(cityId, x, y, assignment)`, absolute `SetSpecialistCount(cityId, specialistName, count)`, and boolean `SetManualSpecialists(cityId, enabled)` implemented; reset citizens, avoid growth, and citizen focus remain separate commands | Shared city worked/locked collections, specialist allocations, and canonical `reassignPopulation()` through focused `HeadlessGameEngine` handlers | Membership-derived civilization, current turn, city ownership, non-puppet/non-resistance state, canonical tile legality or specialist identity/capacity, and canonical free population; client population, capacity, yields, and legality claims are absent | Projection v7 exposes each owned city's allowlisted assignable coordinates plus canonical specialist current/capacity pairs and manual-specialist state; avoid-growth and focus flags remain | `CityScreen` tile controls and specialist `+`/`-`/auto/manual controls route opened v3 games through the session/command bus without local mutation. Disabling manual mode performs canonical worker reassignment. Reset, avoid-growth, and focus controls remain unsupported for v3; local, hotseat, and legacy behavior is unchanged | **Partial** |
