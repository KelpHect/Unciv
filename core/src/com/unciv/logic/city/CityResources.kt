...
object CityResources {
    ...
    fun getResourcesGeneratedByCity(city: City): ResourceSupplyList {
        val modifiers = getResourceModifiers(city)
        @LocalState val cityResources = getResourcesGeneratedByCityNotIncludingBuildings(city, modifiers)
        cityResources += getCityResourcesGeneratedFromUniques(city, modifiers, false)
    ...
    fun getCityResourcesAvailableToCity(city: City): ResourceSupplyList {
        ...
        cityResources.add(getCityResourcesGeneratedFromUniques(city, modifiers, true))
    ...
    /** Does not contain any resources for which there is no specific modifier (==1) */
    @Readonly
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.PercentResourceProduction)) {
            ...
            for (resource in resources) {
                if (resource.matchesFilter(unique.params[1], city.state))
                    modifiers[resource.name] = (modifiers[resource.name] ?: 1f) + bonus
            }
        }
        return modifiers
    }
    ...
    @Readonly
    private fun getResourcesGeneratedByCityNotIncludingBuildings(city: City, modifiers: Map<String, Float>): ResourceSupplyList {
        val cityResources = ResourceSupplyList()

        cityResources.add(getResourcesFromTiles(city, modifiers))
        cityResources.add(getResourceFromUniqueImprovedTiles(city, modifiers))
    ...
    @Readonly
    private fun getResourcesFromTiles(city: City, modifiers: Map<String, Float>): ResourceSupplyList {
        ...
        resourceSupplyList.applyModifiers(modifiers)
        return resourceSupplyList
    }

    @Readonly
    private fun getResourceFromUniqueImprovedTiles(city: City, modifiers: Map<String, Float>): ResourceSupplyList {
    ...
    @Readonly
    private fun getCityResourcesGeneratedFromUniques(city: City, modifiers: Map<String, Float>, includeCivUniques: Boolean = true): ResourceSupplyList {
        ...
        buildingResources.applyModifiers(modifiers)
