...
impl PostgresGameRepository {
    pub async fn execute_cast_diplomatic_vote(
        &self,
        worker: &EngineWorkerClient,
        actor_account_id: Uuid,
        envelope: CommandEnvelope,
    ) -> Result<CommandAccepted, CommitError> {
        let candidate_civilization_id = match &envelope.command {
            crate::GameCommand::CastDiplomaticVote {
                candidate_civilization_id,
            } => candidate_civilization_id.clone(),
        ...
        let proposal = worker
            .cast_diplomatic_vote(
                ...
                CastDiplomaticVoteIntent {
                    actor_civilization_id: &actor_civilization_id,
                    candidate_civilization_id: candidate_civilization_id.as_deref(),
                },
            )
            .await
            .map_err(|error| match error {
                ...
                other => {
                    eprintln!(
                        "authoritative worker CastDiplomaticVote transport/protocol failure: {other}"
                    );
