...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/set-unit-exploration",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = SetUnitExplorationRequest,
    responses(
...
pub(super) async fn set_unit_exploration(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<SetUnitExplorationRequest>,
    ...
    let accepted = state
        ...
        .execute_set_unit_exploration(
            ...
            CommandEnvelope {
                ...
                command: GameCommand::SetUnitExploration {
...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/cancel-unit-movement-order",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = CancelUnitMovementOrderRequest,
    responses(
        (status = 200, body = unciv_authoritative_server::CommandAccepted),
        (status = 401, body = ErrorResponse), (status = 403, body = ErrorResponse),
        (status = 404, body = ErrorResponse), (status = 409, body = ErrorResponse),
...
pub(super) async fn cancel_unit_movement_order(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<CancelUnitMovementOrderRequest>,
) -> Result<Json<unciv_authoritative_server::CommandAccepted>, ApiError> {
    let actor = authenticated_account(&state, &headers).await?;
    let accepted = state
        .repository
        .execute_cancel_unit_movement_order(
            &state.worker,
            actor.id,
            CommandEnvelope {
                protocol_version: PROTOCOL_VERSION,
                ...
                expected_revision: request.expected_revision,
                client_observed_state_hash: request.client_observed_state_hash,
                command: GameCommand::CancelUnitMovementOrder {
                    unit_id: request.unit_id,
                },
            },
        )
