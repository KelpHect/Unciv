...
| End turn and AI | `EndTurn` / `HeadlessGameEngine.endTurn` | Membership-derived civilization; canonical `playerId` and current-turn checks; unit and HTTP rejection tests | Shared v3 transport/command bus reconciles revision and projection; world-screen call site not migrated | Server/client foundation complete; UI/full projection pending |
...
| Unit movement | `MoveUnit` / `HeadlessGameEngine.moveUnit` | Session/membership-derived civilization; current-turn, durable unit ID ownership, map bounds, reachability, and enterability checks; deterministic engine and cross-process HTTP tests | Shared v3 transport/command bus uses permitted unit IDs/coordinates, safe retry, stale refresh, and reconciliation; world-screen call site not migrated | Server/client foundation complete; UI/full projection pending |
