...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/air-sweep",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = AirSweepRequest,
    responses(
...
pub(super) async fn air_sweep(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<AirSweepRequest>,
...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/launch-nuclear-strike",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = LaunchNuclearStrikeRequest,
    responses(
        (status = 200, body = unciv_authoritative_server::CommandAccepted),
        (status = 401, body = ErrorResponse), (status = 403, body = ErrorResponse),
        (status = 404, body = ErrorResponse), (status = 409, body = ErrorResponse),
...
pub(super) async fn launch_nuclear_strike(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<LaunchNuclearStrikeRequest>,
) -> Result<Json<unciv_authoritative_server::CommandAccepted>, ApiError> {
    let actor = authenticated_account(&state, &headers).await?;
    let accepted = state
        .repository
        .execute_launch_nuclear_strike(
            &state.worker,
            actor.id,
            CommandEnvelope {
                protocol_version: PROTOCOL_VERSION,
                ...
                expected_revision: request.expected_revision,
                client_observed_state_hash: request.client_observed_state_hash,
                command: GameCommand::LaunchNuclearStrike {
                    unit_id: request.unit_id,
                    target_x: request.target_x,
                    target_y: request.target_y,
                },
...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/bombard-with-city",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = BombardWithCityRequest,
    responses(
        (status = 200, body = unciv_authoritative_server::CommandAccepted),
        (status = 401, body = ErrorResponse), (status = 403, body = ErrorResponse),
        (status = 404, body = ErrorResponse), (status = 409, body = ErrorResponse),
...
pub(super) async fn bombard_with_city(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<BombardWithCityRequest>,
) -> Result<Json<unciv_authoritative_server::CommandAccepted>, ApiError> {
    let actor = authenticated_account(&state, &headers).await?;
    let accepted = state
        .repository
        .execute_bombard_with_city(
            &state.worker,
            actor.id,
            CommandEnvelope {
                protocol_version: PROTOCOL_VERSION,
                ...
                expected_revision: request.expected_revision,
                client_observed_state_hash: request.client_observed_state_hash,
                command: GameCommand::BombardWithCity {
...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/attack-with-unit",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = AttackWithUnitRequest,
    responses(
        (status = 200, body = unciv_authoritative_server::CommandAccepted),
        (status = 401, body = ErrorResponse), (status = 403, body = ErrorResponse),
        (status = 404, body = ErrorResponse), (status = 409, body = ErrorResponse),
...
pub(super) async fn attack_with_unit(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<AttackWithUnitRequest>,
) -> Result<Json<unciv_authoritative_server::CommandAccepted>, ApiError> {
    let actor = authenticated_account(&state, &headers).await?;
    let accepted = state
        .repository
        .execute_attack_with_unit(
            &state.worker,
            actor.id,
            CommandEnvelope {
                protocol_version: PROTOCOL_VERSION,
                ...
                expected_revision: request.expected_revision,
                client_observed_state_hash: request.client_observed_state_hash,
                command: GameCommand::AttackWithUnit {
                    unit_id: request.unit_id,
                    target_x: request.target_x,
                    target_y: request.target_y,
                },
...
#[utoipa::path(
    post,
    path = "/api/v3/games/{game_id}/commands/paradrop-unit",
    params(("game_id" = uuid::Uuid, Path)), security(("bearer_auth" = [])),
    request_body = ParadropUnitRequest,
    responses(
        (status = 200, body = unciv_authoritative_server::CommandAccepted),
        (status = 401, body = ErrorResponse), (status = 403, body = ErrorResponse),
        (status = 404, body = ErrorResponse), (status = 409, body = ErrorResponse),
...
pub(super) async fn paradrop_unit(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(game_id): Path<uuid::Uuid>,
    Json(request): Json<ParadropUnitRequest>,
) -> Result<Json<unciv_authoritative_server::CommandAccepted>, ApiError> {
    let actor = authenticated_account(&state, &headers).await?;
    let accepted = state
        .repository
        .execute_paradrop_unit(
            &state.worker,
            actor.id,
            CommandEnvelope {
                protocol_version: PROTOCOL_VERSION,
                ...
                expected_revision: request.expected_revision,
                client_observed_state_hash: request.client_observed_state_hash,
                command: GameCommand::ParadropUnit {
                    unit_id: request.unit_id,
