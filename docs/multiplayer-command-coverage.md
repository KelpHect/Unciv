...
| Citizen and specialist assignment | `SetCityTileAssignment(cityId, x, y, assignment)` implemented with closed `unworked`/`worked`/`locked` states; `SetSpecialistCount` and `SetCityAutomation` remain | Shared city worked/locked collections and stat recalculation through `HeadlessGameEngine.setCityTileAssignment`; specialist and automation handlers remain | Membership-derived civilization, current turn, city ownership, non-puppet/non-resistance state, canonical coordinate/ownership/range, not another city's worker, yield, blockade, and free population; client population and legality claims are absent | Projection v6 exposes only each owned city's allowlisted assignable coordinates and worked/locked state; specialist counts, yields, and automation flags remain | `CityScreen` worked-icon click/double-click and `CityScreenTileTable` lock/unlock route opened v3 games through the session/command bus without local mutation. Specialist/automation UI remains unsupported for v3; local, hotseat, and legacy behavior is unchanged | **Partial** |
