...
class CityStats(val city: City) {
    ...
    private fun getStatsFromTradeRoute(): Stats {
        ...
        if (city != capitalForTradeRoutePurposes && city.isConnectedToCapital()) {
            ...
            for (unique in city.matchingUniquesSequence(UniqueType.StatsFromTradeRoute))
                stats.add(unique.stats)
            val percentageStats = Stats()
            for (unique in city.matchingUniquesSequence(UniqueType.StatPercentFromTradeRoutes))
    ...
    fun getStatConversionRate(stat: Stat): Float {
        ...
        val conversionUnique = city.civ.matchingUniquesSequence(UniqueType.ProductionToStatConversionBonus).firstOrNull { it.params[0] == stat.name }
    ...
    private fun getStatsFromUniquesBySource(): StatTreeNode {
        ...
        val cityStateStatsMultipliers = city.civ.matchingUniquesSequence(UniqueType.BonusStatsFromCityStates).toList()
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.StatsPerCity))
            if (city.matchesFilter(unique.params[1]))
                addUniqueStats(unique)

        // "[stats] per [amount] population [cityFilter]"
        for (unique in city.matchingUniquesSequence(UniqueType.StatsPerPopulation))
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.StatsFromCitiesOnSpecificTiles))
    ...
    private fun getStatsPercentBonusesFromUniquesBySource(currentConstruction: IConstruction): StatTreeNode {
        ...
        val uniquesToCheck =
            when {
                currentConstruction is BaseUnit ->
                    city.matchingUniquesSequence(UniqueType.PercentProductionUnits)
                currentConstruction is Building && currentConstruction.isAnyWonder() ->
                    city.matchingUniquesSequence(UniqueType.PercentProductionWonders)
                currentConstruction is Building && !currentConstruction.isAnyWonder() ->
                    city.matchingUniquesSequence(UniqueType.PercentProductionBuildings)
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.StatPercentFromReligionFollowers))
        ...
        if (currentConstruction is Building
            ...
            for (unique in city.matchingUniquesSequence(UniqueType.PercentProductionBuildingsInCapital))
                addUniqueStats(unique, Stat.Production, unique.params[0].toFloat())
        }
    ...
    fun updateCityHappiness(statsFromBuildings: StatTreeNode,
        ...
        for (unique in civInfo.matchingUniquesSequence(UniqueType.UnhappinessFromCitiesPercentage))
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.UnhappinessFromPopulationTypePercentageChange))
            if (city.matchesFilter(unique.params[2]))
    ...
    private fun updateFinalStatList(currentConstruction: IConstruction, calculateGrowthModifiers: Boolean = true) {
        ...
        for ((unique, statToBeRemoved) in city.matchingUniquesSequence(UniqueType.NullifiesStat)
        ...
        val growthNullifyingUnique = city.matchingUniquesSequence(UniqueType.NullifiesGrowth).firstOrNull()
    ...
    private fun calcFoodEaten(): Float {
        ...
        for (unique in city.matchingUniquesSequence(UniqueType.FoodConsumptionByPopulation)) {
