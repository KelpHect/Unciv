...
| Unit movement | `MoveUnit(unitId, destination)` implemented for exact current-turn movement and `SwapUnits(unitId, destination)` implemented for compatible friendly occupancy; persistent multi-turn orders remain separate typed commands | Shared `UnitMovement.moveToTile` and `swapMoveToTile(..., keepEscorting = true)` through focused `HeadlessGameEngine` handlers | Membership-derived current actor/turn; projected own-unit ID and explored destination preflight; canonical ownership, bounds, current-turn reachability, movement, terrain, occupancy, friendly-unit compatibility, and escort rules | Stable unit IDs, explored map coordinates, resulting positions/movement, and visibility changes; server-projected legal movement/swap destinations remain | Opened-v3 world-map overlay clicks, single-tap movement, and swap controls submit through the session/command bus and never mutate local `MapUnit` state; multi-unit moves serialize command-by-command. Local, hotseat, and legacy behavior is unchanged | **Partial: multi-turn orders and projected legal destinations remain** |
